...
async function run() {
  if (false) {
    ...
    logger.info(
      "Manually delete this check to get around this but please don't unless you really know what you're doing."
    );
    process.exit(1);
  }
  ...
  const envsToDelete: Environment[] = (
    await space.getEnvironments()
  ).items.filter((it) => migrationEnvNames.includes(it.name));
